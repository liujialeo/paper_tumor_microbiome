...
#生成颜色
multiColorMapping <- function(x, type, colors = c("#115ca6", "#ef2a2e"), log = T){
...
pathway_info <- readxl::read_excel(path = "NIHMS957693-supplement-7.xlsx", 
                                   sheet = "TGF-Beta") #读取Excel表格里哪个sheet
pathway_info <- subset(pathway_info, `OG/TSG` != "NA")[1:7,] 
pathway_info[1,]
...
subtype_in_fig4 <-  c( "COAD_high","COAD_low","ESCA_high","ESCA_low","LIHC_high","LIHC_low", "PAAD_high","PAAD_low","STAD_high",   "STAD_low"  
...
#                            ), nm = subtype_in_fig4)
subtype_group_in_fig4 <- setNames(c(rep("CNS", 2), rep("Head & Neck",2), rep("Endocrine",2), rep("Thoracic",2), 
                           rep("Breast", 2), rep("Gastrointestinal", 2)), nm = subtype_in_fig4)
...
sample_used <- rownames(data_pathway)[data_pathway$`TGF-Beta` > 0 & 
...
ggData_center$multicolor <- multiColorMapping(ggData_center$value, 
                                              row_annot$Type[match(ggData_center$Gene, 
                                                                  row_annot$Gene)], 
                                              colors = list(OG = "#ef2a2e", TSG = "#115ca6", Other = "grey30"))
...
p_left <- ggplot(row_annot) +
  ...
  scale_fill_manual(values = c(OG =  "#ef2a2e", TSG = "#115ca6", Other = "white")) +
  theme_void()
p_left
...
ggsave(filename = "TGF_alteration.pdf", 
       width = 12, 
       height = 10)
```
